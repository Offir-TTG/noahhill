  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {tiles.map(({ href, label, desc, icon: Icon, count }) => (
          <Link
            key={href}
            href={href}
          ...
          >
            <div className="flex items-start justify-between">
              ...
              {typeof count === "number" && (
                <span className="font-display text-cream text-2xl">{count.toString().padStart(2, "0")}</span>
              )}
            </div>
            <div className="mt-4">
              <p className="font-display lowercase text-cream text-2xl">{label}</p>
              <p className="text-xs text-cream-dim mt-1">{desc}</p>
            </div>
